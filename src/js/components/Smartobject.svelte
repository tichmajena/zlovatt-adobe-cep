...
{#if obj.is_active}
  <div class="w-full relative flex justify-end pr-12 z-10">
    <button
      on:click={deactivate}
      class="bg-blue-700 px-3 py-1 rounded-full text-white text-sm -mb-3"
      >Get Out</button
    >
  </div>
{/if}
...
<div
  ...
  class:bg-blue-200={obj.is_active}
  class:bg-white={!obj.is_active}
  class="border-4 border-blue-700 p-8 rounded-xl z-0 relative mb-8"
>
  <input
    type="text"
    placeholder="Layer Name"
    class="rounded border-0 outline-none py-2 text-sm focus:outline-none focus:shadow-md focus:ring focus:ring-blue-700 bg-blue-400 absolute -top-6 left-4"
